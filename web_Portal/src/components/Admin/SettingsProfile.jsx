...
import { useGetAdminByIdMutation, useUpdateAdminProfileMutation, useGetAdminProfileQuery } from 'features/apis/admin'
...
const SettingsProfile = () => {
  ...
  return (
    <>
      <Text
        className="text-3xl sm:text-[26px] md:text-[28px] text-gray-900_02 mb-[10px] !font-poppins"
      ...
      <div className="h-full flex flex-col bg-white-A700 rounded-[30px] p-5">
        <div className="flex flex-col">
          <div className="text-[18px] font-bold text-[#4C535F] !font-poppins">
            Your Profile Picture
          </div>
          <label
            htmlFor="avatar"
            className="p-5 border relative border-dashed cursor-pointer border-[#000] w-fit flex items-center flex-col mt-3 rounded-2xl"
          >
            <Img
              className="h-[110px] rounded-md w-[110px] object-cover border"
              src={data?.data?.profile_picture_url ? `${process.env.REACT_APP_LOCAL_URL}${data?.data?.profile_picture_url}` : 'images/default.png'}
              alt="ellipseTwentyFive"
            />
            <Button
              className="absolute border-[3px] border-solid border-white-A700 bottom-[4%] sm:bottom-[23%]
                          flex h-[34px] items-center justify-center left-[90px] w-[34px]"
              shape="circle"
              size="sm"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-1 gap-3 mt-7">
            <div className="flex flex-col">
              <label htmlFor="fullName" className="text-[#4C535F] font-bold !font-poppins">Full name</label>
              <Input type="name" id="fullName"
                className="border border-solid p-[0.95rem] mt-3 rounded-[8px] w-full !font-poppins"
            ...
            <div className="flex flex-col">
              <label htmlFor="email" className="text-[#4C535F] font-bold !font-poppins">Email</label>
              <Input type="email" id="email"
                className="border border-solid p-3 mt-3 rounded-[8px] w-full !font-poppins"
                style={{ backgroundColor: "rgba(19, 19, 26, 0.12)" }}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-1 gap-3 mt-7">
            {/* <div className="flex flex-col">
                          ...
                          <Input type="name" id="username"
                            className="border border-solid p-[0.95rem] mt-3 rounded-[8px] w-full"
                            style={{ backgroundColor: "rgba(19, 19, 26, 0.12)" }}
                            placeholder="Please enter your username"
                          />
                        </div> */}
            <div className="flex flex-col">
              <label htmlFor="no" className="text-[#4C535F] font-bold !font-poppins">Phone Number</label>
          ...
          <div className="flex flex-row mt-7 gap-5">
            <div className="flex">
              <button className={`bg-[#743C95] flex items-center flex-row gap-2 w-full text-[#fff] 
                            p-3 rounded-lg font-semibold ${isLoading && "opacity-60 cursor-not-allowed"}`}
                onClick={handlerProfile}
                disabled={isLoading}
              >
                <span className='w-100 !font-poppins'>Update Profile</span>
                {isLoading && <Loader widthAlign={true} customStyle={'!h-full !w-fit'} />}
              </button>
            </div>
            {/* <div className="flex">
