...
const Items = ({ allMentors, check,getMentorData, isCheck, toggleHandler }) => {
  const [updateAnyField] = useUpdateAnyFieldMutation()


  const handlerUpdateApprove = async (decision,id) => {
    let payload = {
      _id : id,
      approved:decision
    }
    ...
    <>
      {allMentors?.map((item, index) => (
        <tr className="overflow-hidden" key={index}>
          ...
          {check === 'mentorManage' ? (<>
            <td className="text-center">{item?.mentorModel?.mentorFeilds[0]}</td>
            ...
            <td className="text-center">
              <div className="flex items-center justify-center">
                <label className="toggle-container w-[109px]">
                  <input
                    type="checkbox"
                    checked={item?.approved}
                    onChange={() => handlerUpdateApprove(!item?.approved ? true : false,item?._id)}
                  />
                  <span className={`toggle-slider approve-toggle p-[10px] ${item?.approved ? 'flex justify-start items-center text-[13px] text-[#fff]' : 'flex justify-end items-center text-[11px]'}`}>
                    {item?.approved ? 'Approve' : 'Disapprove'}
                  </span>
                </label>
              </div>
            </td>
          ...
          <td className="text-base text-purple-700 underline sm:p-[13px] sm:px-[18px]">
            <Link to={`/profile/mentors/${item?.mentorModel?._id}`}>View Details</Link>
          </td>
        </tr>
      ))}
    </>
