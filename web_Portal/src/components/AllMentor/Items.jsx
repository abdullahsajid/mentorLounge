...
const Items = ({allMentors,check,isCheck,toggleHandler}) => {
  return (
    <>
      {allMentors?.map((item,index) => (
        <tr className="overflow-hidden"key={index}>
            ...
            {check === 'mentorManage' ? (<>
                <td className="text-center">{item?.mentorFeilds[0]}</td>
            ...
            <td className="text-center">{item?.verifiedMentor ? "Verified" : "UnVerified"}</td>
            <td className="text-center">{item?.mentorFeilds[0]}</td></>)}
            <td className="text-base text-purple-700 underline sm:p-[13px] sm:px-[18px]">
            <Link to={`/profile/mentors/${item?._id}`}>View Details</Link>  
            </td>
        </tr>
        ))}
    </>
