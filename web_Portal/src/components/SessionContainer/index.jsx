...
const SessionContainer = ({ name, img, title, startTime, endTime, meeting }) => {
    ...
    var meetingNumber = meeting?.id
    var passWord = meeting?.password
    var role = 1
    var userName = mentorData?.data?.user?.name
    ...
    // const getNav = () => {
    //     navigation(`/mtr-meeting/${meeting?.id}/${meeting?.password}`)
    // }
            ...
            <div className="flex sm:justify-between w-full">
                <div className="flex items-center flex-col w-full">
                    ...
                    {true && <div className="flex gap-3 w-full">
                        <a
                            // href={`http://localhost:3001/meeting/${1}/${meeting?.id}/${meeting?.password}/${mentorData?.data?.user?.name}`}
