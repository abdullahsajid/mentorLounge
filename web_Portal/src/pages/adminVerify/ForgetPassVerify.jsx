...
import { setVerifyForPassword, setForgetFields } from 'features/admin/adminSlice';
...
const ForgetPassVerify = () => {
    ...
    const handlerVerify = async (e) => {
        ...
        if (code === '') {
            ...
            if (data.status === 'Success') {
                localStorage.setItem('loungeForgetCookie', JSON.stringify(data?.token))
                dispatch(setVerifyForPassword(false))
                dispatch(setForgetFields(true))
    ...
    return (
        <div className="w-full h-screen flex justify-center items-center fixed z-[110] top-0"
